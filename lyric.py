...
get_next_index = lambda x,y:y.index((list(filter(lambda k:False if not type(k[0]) == float else tolerance[0]*(10**(-offset)) <= tag_to_sec(k[0])-x[0] <= tolerance[1]*(10**(-offset)), y)) + [y[-1]])[0])+1
plyric_to_ptlyric = lambda x,y:[(i[0] if type(i[0]) != float else y[-1][0] if get_next_index(i,y) >= len(y) else y[get_next_index(i,y)][0] - (10**(-offset)), i[1]) for i in x]
plyric_to_lyric = lambda x:reduce(lambda b,c:b+'\n'+c, map(lambda a:(('[{:.0f}:{:.%sf}]{}' % offset) if type(a[0]) == float else '[{}]{}').format(*(divmod(a[0], 60) if type(a[0]) == float else [a[0]]), a[1]), x))
